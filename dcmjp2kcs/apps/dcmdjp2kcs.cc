 ...
 *
 *
 *  Purpose: Decode JPEG 2000 compressed DICOM file
 *
 */
...
#define OFFIS_CONSOLE_APPLICATION "dcmdjp2kcs"
#endif

static OFLogger dcmdjp2kcsLogger = OFLog::getLogger("dcmtk.apps." OFFIS_CONSOLE_APPLICATION);
...
LICENSE_FILE_DECLARE_COMMAND_LINE_OPTIONS
  ...
  cmd.addGroup("processing options:");
    ...
    cmd.addSubGroup("workaround options for incorrect JPEG 2000 encodings:");
      cmd.addOption("--workaround-incpl",       "+wi",    "enable workaround for incomplete JPEG 2000 data");
...
LICENSE_FILE_EVALUATE_COMMAND_LINE_OPTIONS
    ...
    OFLOG_DEBUG(dcmdjp2kcsLogger, rcsid << OFendl);
    ...
    {
      OFLOG_WARN(dcmdjp2kcsLogger, "no data dictionary loaded, "
    ...
    {
      OFLOG_FATAL(dcmdjp2kcsLogger, "invalid filename: <empty string>");
      return 1;
    }

    DcmFileFormat fileformat;

    OFLOG_INFO(dcmdjp2kcsLogger, "reading input file " << opt_ifname);

    OFCondition error = fileformat.loadFile(opt_ifname, opt_ixfer, EGL_noChange, DCM_MaxReadLength, opt_readMode);
    if (error.bad())
    {
      OFLOG_FATAL(dcmdjp2kcsLogger, error.text() << ": reading file: " <<  opt_ifname);
      return 1;
    }

    DcmDataset *dataset = fileformat.getDataset();

    OFLOG_INFO(dcmdjp2kcsLogger, "decompressing file");
    ...
    if (error.bad())
    {
      OFLOG_FATAL(dcmdjp2kcsLogger, error.text() << ": decompressing file: " <<  opt_ifname);
      if (error == EC_CannotChangeRepresentation)
        OFLOG_FATAL(dcmdjp2kcsLogger, "input transfer syntax " << original_xfer.getXferName() << " not supported");
      return 1;
    }

    if (! dataset->canWriteXfer(opt_oxfer))
    {
      OFLOG_FATAL(dcmdjp2kcsLogger, "no conversion to transfer syntax " << opt_oxferSyn.getXferName() << " possible");
      return 1;
    }

    OFLOG_INFO(dcmdjp2kcsLogger, "creating output file " << opt_ofname);
    ...
    if (error != EC_Normal)
    {
      OFLOG_FATAL(dcmdjp2kcsLogger, error.text() << ": writing file: " <<  opt_ofname);
      return 1;
    }

    OFLOG_INFO(dcmdjp2kcsLogger, "conversion successful");
